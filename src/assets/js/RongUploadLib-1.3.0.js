...
(function (RongIMLib) {
    var RongUploadLib = (function () {
        function RongUploadLib(imgOpts, fileOpts) {
            ...
            plScript.src = RongIMLib.RongIMClient._memoryStore.depend.fileUploadURL;
            plScript.onload = plScript.onreadystatechange = function () {
                if (!this.readyState || this.readyState == 'loaded' || this.readyState == 'complete') {
                    var qiniuScript = document.createElement('script');
                    qiniuScript.src = RongIMLib.RongIMClient._memoryStore.depend.fileQNURL;
		...
		RongUploadLib.getFileUrl = function (info,callback) {
			var url = "";
			if(info.rc_url.type == 0){
				url = RongIMLib.RongIMClient._memoryStore.depend.fileServer + info.rc_url.path;
			}else{
				url = info.rc_url.path;
			}
			callback.onSuccess(url);
        };
		RongUploadLib.uploadAjax = function (base64, callback) {
		   var url = RongIMLib.RongIMClient._memoryStore.depend.postImageUrl;   
        ...
        RongUploadLib.prototype.postImage = function (base64, file, conversationType, targetId, callback) {
            var me = this;
			 RongUploadLib.uploadAjax(base64, function (ret) {
				var opt = { uploadType: 'IMAGE', info: ret, isBase64Data: true,fileName:ret.name };
        ...
        RongUploadLib.prototype.createMessage = function (option, file, callback) {
            ...
            switch (option.uploadType) {
                case 'IMAGE':
                    ...
                    RongIMLib.RongUploadLib.getFileUrl(option.info, {
                        onSuccess: function (data) {
                            if (option.isBase64Data) {
                                RongUploadLib.imageCompressToBase64(file, function (content) {
                                    msg = new RongIMLib.ImageMessage({ content: content, imageUri: data });
                                    callback(msg);
                                });
                            }
                            else {
                ...
                case 'FILE':
                    RongIMLib.RongUploadLib.getFileUrl(option.info, {
                        onSuccess: function (data) {
                            ...
                            msg = new RongIMLib.FileMessage({ name: file.oldName, size: file.size, type: type, fileUrl: data });
                            callback(msg);
                        },
                        onError: function (error) { }
                    });
